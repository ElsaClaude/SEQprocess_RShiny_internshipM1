...
#set programs path
program.dir="/net/travail/elclaude/soft"

#set references path
reference.dir="/net/travail/elclaude/stageM1/data"
...
#------------------------------ **QualityCheck** -------------------------------- 
  # fastqc
  fastqc.path=file.path(program.dir, "miniconda2/bin", "fastqc")

#------------------------------ **Trimming** -------------------------------------
  # trim
  trim_galore.path=file.path(program.dir,"miniconda2/bin","trim_galore")
  trim.quality=30
  trim.clip_R1=13
  trim.clip_R2=13
  
  # cutadapt
  cutadapt.path=file.path(program.dir, "miniconda2/bin/cutadapt")
  m=17 # minimum read length
  adapt.seq="TGGAATTCTCGGGTGCCAAGG"
  
#------------------------------ **Alignment** ------------------------------------
  # bwa
  bwa.path=file.path(program.dir, "miniconda2/bin", "bwa")
  
  # samtools 
  samtools.path=file.path(program.dir, "miniconda2/bin", "samtools")
  
  # STAR
  STAR.path=file.path(program.dir, "miniconda2/bin/STAR")
  sjdbOverhang=100
  ...
  # tophat2
  tophat2=file.path(program.dir, "miniconda2/bin/tophat2")
  
  # bowtie2
  Sys.setenv("PATH"=paste(Sys.getenv("PATH"), "/data/program/bin/", sep=":"))
  bowtie2.path=file.path(program.dir, "miniconda2/bin/bowtie2")
...
#------------------------------ **Re-alignment** -------------------------------
  # GATK
  GATK.path=file.path(program.dir, "GATK/gatk-4.1.2.0/gatk-package-4.1.2.0-local.jar") # variant calling too
...
#------------------------------ **VariantCalling** -------------------------------
  ...
  # varscan2 : OK ???
  Varscan.path=file.path(program.dir, "varscan/share/varScan-2.4.3-2/VarScan.jar")
  mapQ=1
  min_tumor_freq=0.1
...
#------------------------------ **Annotation** -----------------------------------
  ...
  vcf2annovar.pl=file.path(program.dir, "ANNOVAR/annovar/convert2annovar.pl")
  format = "vcf4"
  coverage = 0
  
  table_annovar.pl=file.path(program.dir, "ANNOVAR/annovar/table_annovar.pl")
  ref = "hg38"
...
##CHROMS ?????? -> chrom.fa= file.path(reference.dir, "[[path to referece directory]]/20150407_reference_files/chroms")

## Reference index #######
# bwa
bwa.idx=file.path(reference.dir, "bwa_index")
